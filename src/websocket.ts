...
export class SocketClient {
  ...
  constructor({project}) {
    ...
    this.socket = io(SERVER_URL, {
      ...
      auth: { token: sessionStore.getState().token ,authKey:sessionStore.getState().authKey}, // make sure token exists
    });
...
this.socket.on("disconnect", async () => {
  ...
  if (!token || !authKey) {
    ...
    try {
      ...
      const { token: newToken, authKey: newAuthKey } = sessionStore.getState();
...
// asdfgfdawerdf=sdfgdsregf
    this.socket.on("envupdated", async (msg) => {
      setTimeout(async () => {
        ...
        if (config.onSync) {
          config.onSync({ list: envs.list, changes: getenvObject(envs.changes) });
        }
